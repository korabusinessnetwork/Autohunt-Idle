...
9. ~~**(Amendado)** Pontos de atributo são **auto-alocados por padrão** (distribuição balanceada, sem exigir touch do jogador) — compatível com o Princípio nº1 (zero esforço). Jogador pode realocar manualmente a qualquer momento~~ → **REVERTIDO PELO DONO em 2026-08-13.** Cada level up concede **1 ponto**, e **quem gasta é o jogador** — não existe distribuição automática. O que atende o Princípio nº1 agora é o **selo de pontos livres no ícone de atributos**: o convite aparece sem exigir leitura, e ignorá-lo não quebra nada, só deixa ponto guardado. Ver `docs/03_REGRAS_DE_NEGOCIO/` §3 e `supabase/migrations/20260829_atributos_manuais.sql`
