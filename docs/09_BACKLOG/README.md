...
| # | Item | Detalhe |
|---|---|---|
| ~~D1~~ | ~~**Migrations nunca executadas**~~ — **resolvido em 2026-08-11** | As 10 migrations agora aplicam contra um Postgres 16 de verdade, e um teste de fumaça exercita a lógica do jogo ponta a ponta: `./scripts/pg-local.sh`. A ressalva que ficou — o stub reproduz `auth.uid()` por variável de sessão, não por JWT, então **RLS não é exercitada** — foi **verificada à mão em 2026-08-12**, contra o projeto real: duas contas, sete tabelas, leitura e escrita cruzadas, mais o papel `anon`. A política estava certa. A ressalva **não deixa de existir por isso**: continua sem teste automático, e por isso virou passo recorrente do checklist de release §4, não item riscado. |
...
| ~~D3~~ | ~~**Arte é placeholder**~~ — **resolvido em 2026-08-12** | Os assets voltaram (302 PNGs, 9 levas, escala 8) e entraram pelo `src/game/atlas.ts`. A aposta de isolar a camada de sprite se pagou: **`motor.ts` não mudou uma linha**, e `mundo.ts` só ganhou temporizadores de pose — e, em 2026-08-14, um acumulador de fase de passo, pelo mesmo motivo (o renderizador não recebe tempo; quem tem relógio protegido por `DT_MAXIMO` é o mundo). As silhuetas geométricas continuam no código como fallback do carregamento — o jogo abre mesmo sem a arte. Ver `specs/build-fase-3j-arte-real.md` e `docs/02_DESIGN_SYSTEM/inventario-de-arte.md`. Sobraram duas ressalvas, registradas como D16 e D17. |
| D16 | **A arte não cobre 4 dos 7 slots, nem as pedras** | Capacete, armadura, luva e bota não têm ícone por raridade no pacote entregue — usam o ícone de slot, sem escalada visual de tier. As três pedras não têm ícone nenhum (`arteDoItem` devolve `null` e a interface mostra o rótulo de texto). Estas duas seguem sendo pedido de arte. **Correção de 2026-08-14:** a terceira ressalva desta linha — "as skins vieram numa pose só" — estava classificada aqui como *"não quebra tela; é pedido para a próxima rodada de arte, não código"*, e a classificação era **falsa**. O dono relatou como bug (*"quando eu equipo a skin o boneco fica travado de animação"*), e era literal: `arteDoHeroi` descartava a pose sempre que havia skin, então equipar congelava o boneco. Foi resolvido **por código**, sem arte nova: movimento procedural em `src/game/sprites.ts` (`deslocamentoDoHeroi`). O que restou em aberto é outra coisa — a skin não tem variação de silhueta por ação —, e essa sim custa arte. Lição registrada: "é pedido de arte" foi o rótulo que manteve um defeito visível parado. |
...
| D11 | **4 ameaças seguem ABERTAS, nenhuma por falta de código** | **Atualizado em 2026-08-12:** RLS sob JWT real saiu da lista — foi exercitada contra o projeto Supabase assim que ele existiu, e a política estava certa (ameaça 7.5, agora MITIGADA). Ficaram calibragem do preço da fortificação (P6), calibragem da trilha do passe, webhook de gateway (P3) e replay de callback de anúncio (P2). Listadas com impacto em `docs/11_SEGURANCA/README.md`. |
...
| R3 | ~~Auto-alocação desliga no primeiro respec manual~~ → **superada em 2026-08-13**: a auto-alocação saiu inteira do jogo, e o level up passou a dar **1 ponto**, não 3. Quem gasta é o jogador | pedido do dono; a auto-alocação nasceu para servir o Princípio nº 1 e acabou tirando toda decisão da tela de atributos | `20260829_atributos_manuais.sql` devolve o corpo antigo de `auto_alocar_atributos` e o `* 3` de `pontos_ganhos_ate` |
