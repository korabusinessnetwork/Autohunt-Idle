...
| D5 | **Docs 03, 04, 05, 06 e 07 seguem vazios** | Regras de negócio, modelagem, fluxos, componentes e APIs continuam com o README de esqueleto. O schema e o contrato das RPCs existem em código e nas migrations, mas ainda não foram espelhados na documentação — e `CLAUDE.md` diz que a documentação é que prevalece. **Correção de 2026-08-11:** a doc 11 estava indevidamente nesta lista. `docs/11_SEGURANCA/README.md` não é esqueleto — são 97 linhas de plano real vindas da fundação. O problema dela é outro, e está em D9. |
| D9 | **O plano de segurança precede o sistema que ele deveria cobrir** | `docs/11_SEGURANCA/README.md` foi escrito na fundação, antes de existir migration nenhuma. Hoje há 11 migrations, contrato de RPC, GRANT por coluna, RNG determinístico e uma economia de duas moedas — nada disso está refletido no modelo de ameaças, e o checklist de release não aponta para nenhum teste que exista. Consolidar isso é a próxima rodada do loop. |
...
| D8 | **Mercado P2P entre jogadores não foi construído — e não é esquecimento** | `specs/mercado-diamante.md` tem os critérios 5 a 7 (troca entre jogadores). `memory/restrictions.md` exige plano de segurança **específico** escrito antes. O plano geral existe (`docs/11_SEGURANCA/README.md`), mas ele próprio manda "reabrir esta seção de compliance por completo antes de construir" — verificação de idade mais forte, moderação e revisão legal específica. É esse plano específico que falta, e escrevê-lo é o que destrava a metade que falta da spec. Ver D9. |
